...
def frame_detect_cb(data):
    global detect_frame,detect_line
    detect_frame=data.detect_frame
    detect_line=data.detect_line
...
def depth_image_cb(data):
    ...
    try:
        ...
        image_binary[(depth_frame < 4.9) & (depth_frame > 1.0)] = 255
...
def detector_of_pillar():
    global flag, rgb_image_copy, cv_image,image_binary
    global min_dist_index, point_near, point_cloud,drone_pose,yaw,detect_line
    ...
    try:
        ...
        if rgb_image_copy is not None and 255 in image_binary:
            ...
            if len(contours):
                ...
                print("YAW",yaw)
                if cv.contourArea(contours[0]) > 10000. and dist_to_pillar < 2.5 and not window_detect_flag and ang_obs<math.pi/3:
                    print "detect pillar"
...
def trajectory_publisher(trajectory,height):
    global goal_pose_pub, drone_pose, detect_frame_publisher, frame_detect_flag,point_near
    #flag_corrector_course = False
    if math.atan2(point_near[1],point_near[0])-yaw<0:
        trajectory=list(reversed(trajectory))
    
    while True and detect_line==False:
        if len(trajectory) != 0:
           ...
           # goal_pose.pose.course = ang_obs
            ...
            while True:
                #print ("BRUH")
                if 10>math.sqrt((goal_pose.pose.point.x-drone_pose.pose.position.x)**2+(goal_pose.pose.point.y-drone_pose.pose.position.y)**2)>0.5 or detect_line==False:
...
def main():
    global depth_frame, image_binary, rgb_image, detect_frame_publisher, rgb_integrate, goal_pose_pub, marker_publisher, client,pub_marker
