...
cam_img_topic = "/mono_cam_down/camera_mono/image_raw"                   # топик нижней камеры
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if not detect_frame_flag:
            ...
            r_channel = hls[:, :, 1]
            s_channel = np.zeros_like(r_channel)
            s_channel[(r_channel < 50)] = 255



            # Фильтруем
            # Уменьшаем контуры белых объектов - делаем две итерации
            AllBinary = cv.erode(s_channel, None, iterations = 5)
            ...
            # cv.imshow("asdas", allbinary_copy)
