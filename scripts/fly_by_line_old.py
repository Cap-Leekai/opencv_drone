...
cam_img_topic = "/iris_rplidar/usb_cam/image_rawww"                    #/iris_rplidar/usb_cam/image_raw топик нижней камеры                                       # топик текущей высоты
...
def get_params_server():
    global cam_img_topic, view_result_flag, image_width_px, image_height_px, x_forward, hight_center
    ...
    x_forward = rospy.get_param('~x_forward', x_forward)
    hight_center = rospy.get_param('~hight_center', hight_center)
...
def main():
    ...
    detect_line_pub = rospy.Publisher(frame_detect_topic, frame_detect, queue_size = 10)
    ...
    while not rospy.is_shutdown():
        ...
        if len(contours):
            rospy.loginfo("Detect line!")
            #***#
            detect_line.detect_line = True
            detect_line_pub.publish(detect_line)
            #***#
            ...
            correct_x = (sm_pix_x / pixel_on_meter) + x_forward
            ...
            # if drone_alt > 1.2:
            goal_pose.pose.point.z = hight_center
            # elif drone_alt < 1.3:
